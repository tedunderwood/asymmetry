...
def get_kld_timelines(package):
    ...
    for idx in range(start, end):
        ...
        # note the field names "earliestdate" (instead of inferreddate)
        # and "lastname" (instead of author).

        # The first of these is simply a consequence of merging two data
        # sources; a big random sample dated by pubdate for *the earliest vol in Hathi*
        # much smaller (1/38th) sample of fiction that has real dates of first publication.

        # The second is a move to be maximally conservative in avoiding self-comparisons.
        ...
        for yr in range (floor, ceiling):
            ...
            thisyear = meta.loc[meta.earliestdate == yr, : ]
